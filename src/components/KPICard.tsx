...
export function KPICard({ bank, kpi, showLabel = true }: Props) {
...
export function BankSummaryCard({ bank, kpis = ['lucro_liquido_recorrente', 'roe', 'indice_eficiencia', 'carteira_credito'] }: BankCardProps) {
  const color = BANK_COLORS[bank.ticker] || '#6366f1'

  return (
    <Link href={`/bancos/${bank.ticker.toLowerCase()}`}>
      <div className="bg-slate-900 border border-slate-700/50 rounded-2xl p-6 hover:border-slate-500 hover:bg-slate-800/50 transition-all cursor-pointer group">
        {/* Header */}
